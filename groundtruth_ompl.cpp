...
int main(int argc, char* argv[]) {
    ...
    if (weightStrings.empty()) {
        std::cerr << "No weights loaded from " << inputFile << "\n";
        return 1;
    }

    // 6. Dynamic Output File
    std::string outFileName = "groundTruth_scenario_" + std::to_string(scenario) + ".csv";
    ...
    if (fileExists) {
        std::cout << ">>> Resuming: Found " << processedWeights.size() << " existing entries in " << outFileName << std::endl;
        outFile.open(outFileName, std::ios::app); // Append mode
    } else {
    ...
    for (const auto& wStr : weightStrings) {
        ...
        if (processedWeights.count(cleanW.str())) {
            ...
            if (skipped_count % 10 == 0) std::cout << "\rSkipped " << skipped_count << " existing entries..." << std::flush;
            continue;
        }
        // Reset for new weight
        setup.clear();
        ...
        // --- C. RPS Iterative Convergence Loop ---
        std::cout << "Weight [" << w[0] << ", " << w[1] << ", " << w[2] << "] -> Solving..." << std::endl;
        ...
        if (setup.haveExactSolutionPath()) {
            ...
            std::stringstream cleanW; cleanW << w[0] << ";" << w[1] << ";" << w[2];
