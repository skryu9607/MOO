 ...
 * 
 * Compilation:
 g++ -m64 -O3 Tch_groundtruth_ompl.cpp -o TchgroundTruth \
...
public:
    ...
    ob::Cost motionCost(const ob::State *s1, const ob::State *s2) const override {
        ...
        const std::size_t n = std::min({weights_.size(), c.size()});
        constexpr double Z_LENGTH = 0.486425;
        constexpr double Z_RISK   = 0.0384961;
        constexpr double Z_TIME   = 0.12467;
        const double z_ref[3] = {Z_LENGTH, Z_RISK, Z_TIME};
        double max_term = -std::numeric_limits<double>::infinity();
        for (std::size_t i = 0; i < n; ++i) {  
            const double t_i = weights_[i] * (c[i] - z_ref[i]);
            max_term = std::max(max_term, t_i);
        }
        return ob::Cost(max_term);
    }
private:
...
int main(int argc, char* argv[]) {
    ...
    std::string outFileName = "LINCost_TCH_groundTruth_scenario_"
