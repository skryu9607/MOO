...
class MORRFplanner{
    ...
    start(start), goal(goal), threshold(threshold),num_objectives(number_objectives), num_sub(number_subproblems){
        ...
        for (size_t k = 0; k < num_sub; ++k){
            ...
            std::vector<double> ref = std::vector<double>(num_objectives,1.0);
    ...
    std::vector<double> z_utop = {24.41,0.0};
        ...
        for (size_t k = 0; k < num_sub; ++k){
            ...
            for (std::shared_ptr<Node> NrNode:NrNodes){
                if (isObstacleFree(line(new_node->state,NrNode->state))){
                    ...
                    //std::cout << "z_hat_utop (" << new_node->id << ") : " << z_hat_utop[0] << ", " << z_hat_utop[1] << std::endl;
...
std::vector<double> calculateSegmentCost(const State& s_from, const State& s_to){
    ...
    */
    ...
    for (int i = 0; i < num_steps; ++i){
        ...
        intermediate.x = s_from.x + ratio * (s_to.x - s_from.x);
        intermediate.y = s_from.y + ratio * (s_to.y - s_from.y);
        intermediate_dist = stateDistance(intermediate, State{11.0,13.0}) - radius;
        if (intermediate_dist < 0.0){
            intermediate_dist = 0.01;
        }
        dist += intermediate_dist;
    ...
    //cost[1] = cost[0];
    // Cost[2] : travel time.
    State previous_intermediate = State{s_from.x, s_from.y};
    double speed;
    double Time;
    ...
    for (int i = 1; i <= num_steps; ++i){
        ...
        intermediate.x = s_from.x + ratio * (s_to.x - s_from.x);
        intermediate.y = s_from.y + ratio * (s_to.y - s_from.y);
        if (intermediate.y < 15.0){
            speed = 100.; // Highway
        }
        else{
            speed = 2.;
        }
        distance_segment = stateDistance(intermediate, previous_intermediate);
